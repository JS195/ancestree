...
**`schema.py`.** `SCHEMA_SQL`, `SCHEMA_VERSION`, `ensure_schema(conn)`, `migrate(conn)` (steps `PRAGMA user_version`). The DDL lives in one canonical, versioned place; `auto_vacuum=INCREMENTAL` is set at creation because it cannot be enabled later.
...
- [x] **Benchmark gate:** measure dedup ratio and ingest/read overhead on near-duplicate fixtures; set the Layer-2 default from the data (AD5). → **2.3× less storage for 1.07× ingest; default ON** (`benchmarks/RESULTS.md`).
