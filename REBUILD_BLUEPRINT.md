...
- [x] **Benchmark gate:** measure dedup ratio and ingest/read overhead on near-duplicate fixtures; set the Layer-2 default from the data (AD5). → **2.3× less storage for 1.07× ingest; default ON** (`docs/benchmarks/RESULTS.md`). *(Re-measured at the shipping 16 KiB chunk size: 2.23× on that synthetic random-byte fixture, 2.63× on a mixed real-file corpus — see v2.8 below.)*
