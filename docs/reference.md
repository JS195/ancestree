...
A store is a directory holding one SQLite database. Rules, generation triggers and the `dedup`/`chunk` policy are written in at creation and read back on every later open, so reopening by path is enough. See [Caveats](caveats.md#rules-and-policy-are-set-once) for what cannot be changed afterwards.
