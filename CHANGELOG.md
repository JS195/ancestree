...
- Layer-2 deduplication: near-identical artifacts are stored as chunk deltas against similar chunks already in the pool, using zlib dictionary compression. On a mixed 70 MB corpus of six file types across six revisions the store is 3.93x smaller — 3.13x counting the whole database file rather than the chunk pool, and 2.22x better than Layer 1 alone. On the synthetic near-duplicate fixture, 2.23x. Every number is in `docs/benchmarks/RESULTS.md`, reproducible from the notebooks beside it. Payloads zlib cannot shrink (PNG, parquet, zip) are stored verbatim rather than re-compressed.
