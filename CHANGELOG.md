...
- `reuse_identical` (the node-level policy, formerly `dedup`) and `chunk` are persisted store policy, set once at creation like `rules`, rather than flags to re-pass on every open. "Deduplication" now refers only to storage; reusing an identical node is `reuse_identical`.
